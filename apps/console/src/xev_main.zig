...
fn runAgent(allocator: std.mem.Allocator, args: [][:0]u8) !void {
    ...
    if (message.len == 0) {
        ...
        var agent_instance = try agent.Agent.init(allocator, config, session_id, save_to_rag);
        ...
        while (true) {
            ...
            // Process user input
            agent_instance.run(trimmed) catch |err| {
                std.debug.print("Error: {any}\n", .{err});
            };

            // Save conversation to RAG if enabled
            if (save_to_rag) {
                agent_instance.indexConversation() catch |err| {
    ...
    try agent_instance.run(message);

    // Save to RAG if enabled
    if (save_to_rag) {
        try agent_instance.indexConversation();
    }
}
...
fn runConsole(allocator: std.mem.Allocator, args: [][:0]u8) !void {
    ...
    var bot = try agent.console.MockBot.init(allocator, config, save_to_rag);
    defer bot.deinit();
...
fn runConsoleSync(allocator: std.mem.Allocator, args: [][:0]u8) !void {
    ...
    try agent.console_sync.run(allocator, config, save_to_rag);
}
...
fn runTelegramBot(allocator: std.mem.Allocator, args: [][:0]u8) !void {
    ...
    try agent.telegram_bot_sync.run(allocator, config, save_to_rag);
}
...
fn runWhatsAppBot(allocator: std.mem.Allocator, args: [][:0]u8) !void {
    ...
    try agent.whatsapp_bot.run(allocator, config);
}
...
fn runGateway(allocator: std.mem.Allocator) !void {
    ...
    var g = try agent.gateway.Gateway.init(allocator, config);
    defer g.deinit();
...
fn runVectorDb(allocator: std.mem.Allocator, args: [][:0]u8) !void {
    ...
    var store = agent.vector_db.VectorStore.init(allocator);
    defer store.deinit();
    ...
    if (std.mem.eql(u8, sub_cmd, "list")) {
        ...
        var resp: providers.base.EmbeddingResponse = undefined;
...
fn validateConfig(config: agent.config.Config) !void {
...
fn autoCreateWhatsAppConfig(allocator: std.mem.Allocator) !void {
    ...
    var parsed_config = agent.config.loadFromPath(allocator, config_path) catch |err| {
        if (err == error.FileNotFound) {
    ...
    // Save updated config
    try agent.config.saveToPath(allocator, config, config_path);

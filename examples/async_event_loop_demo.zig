...
pub fn main() !void {
    ...
    try event_loop.addCronJob(
        "morning_digest",
        "Morning Digest",
        "Create a daily summary of all conversations and important events",
        .{
            .kind = .every,
    ...
    try event_loop.addCronJob(
        "health_check",
        "System Health Check",
        "Check all systems, databases, and external services",
        .{
            .kind = .every,
    ...
    try event_loop.addCronJob(
        ...
        "Generate comprehensive weekly usage and performance report",
        .{
            .kind = .every,
...
fn simulateDiscordChats(loop: *AsyncEventLoop) void {
    ...
    while (message_count < 8) : (message_count += 1) {
        ...
        const message = try std.fmt.allocPrint(
            loop.allocator,
            "[Discord] Hey everyone, check out this cool Zig pattern!",
            .{}
        );
        defer loop.allocator.free(message);
...
fn simulateWhatsAppChats(loop: *AsyncEventLoop) void {
    ...
    while (message_count < 6) : (message_count += 1) {
        ...
        const message = try std.fmt.allocPrint(
            loop.allocator,
            "[WhatsApp] Quick question about async programming",
            .{}
        );
        defer loop.allocator.free(message);
...
fn simulateAdminCommands(loop: *AsyncEventLoop) void {
    ...
    try loop.addCronJob(
        "urgent_notification",
        "Urgent Notification",
        "Send urgent notification to all active chats",
        .{
            .kind = .at,
    ...
    const system_message = try std.fmt.allocPrint(
        loop.allocator,
        "[SYSTEM] High load detected on chat servers",
        .{}
    );

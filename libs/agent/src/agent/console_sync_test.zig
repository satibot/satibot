...
// Since the functions in console_sync.zig are private, we need to test them
// by copying the implementations here for testing purposes
...
test "console_sync.isQuitCommand" {
    // Test positive cases
    ...
    // Test negative cases
    try std.testing.expect(!isQuitCommand("exit2"));
    try std.testing.expect(!isQuitCommand("quitting"));
    try std.testing.expect(!isQuitCommand("help"));
    try std.testing.expect(!isQuitCommand(""));
    try std.testing.expect(!isQuitCommand("  "));
    try std.testing.expect(!isQuitCommand("hello world"));
}

test "console_sync.getHistoryPath" {
...
test "console_sync.getHistoryPath no HOME" {
    const allocator = testing.allocator;
...
test "console_sync.saveHistory and loadHistory" {
    const allocator = testing.allocator;
...
test "console_sync.loadHistory empty file" {
    const allocator = testing.allocator;
...
test "console_sync.loadHistory file not found" {
    const allocator = testing.allocator;
...
test "console_sync.loadHistory with whitespace and empty lines" {
    const allocator = testing.allocator;
...
test "console_sync.loadHistory exceeds MAX_HISTORY_LINES" {
    const allocator = testing.allocator;
...
test "console_sync.saveHistory exceeds MAX_HISTORY_LINES" {
    const allocator = testing.allocator;
...
test "console_sync.loadHistory with long lines" {
    const allocator = testing.allocator;
    ...
    const test_path = "long_lines_history";

    // Create file with very long lines that span multiple reads
    const file = try tmp_dir.dir.createFile("long_lines_history", .{});
    defer file.close();

    var long_line = std.ArrayList(u8).initCapacity(allocator, 10000) catch unreachable;
    defer long_line.deinit(allocator);

    // Create a line longer than the read buffer (8192 bytes)
    for (0..10000) |i| {
        try long_line.writer(allocator).print("{}", .{i % 10});
    }

    try file.writeAll(long_line.items);
    try file.writeAll("\n");

...
test "TelegramBot send_chat_action with fake token fails" {
    ...
    // Test that send_chat_action can be called with fake credentials
    // The method may succeed or fail depending on network, but should not crash
    // We just verify the method signature is correct and it doesn't panic
    bot.sendChatAction("fake-token-for-testing", "123456") catch |err| {
        // Expected to fail with fake token - log but don't fail test
        std.debug.print("sendChatAction failed as expected: {any}\n", .{err});
    };
}
...
test "TelegramBot: memory - offset management doesn't leak" {
    ...
    for (0..100) |i| {
        ...
        try std.testing.expectEqual(@as(i64, @intCast(i + 1)), bot.offset);
    }

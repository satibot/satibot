...
const opencode_module = @import("../../agent-opencode.zig");
...
/// Handle /opencode command to send message to OpenCode
fn handleOpencodeCommand(ctx: *TelegramContext, tg_data: TelegramTaskData) !void {
    const message = tg_data.text["/opencode ".len..];
    const opencode_msg = std.mem.trim(u8, message, " \t\r\n");

    if (opencode_msg.len == 0) {
        const error_msg = try std.fmt.allocPrint(ctx.allocator, "❌ Usage: `/opencode <message>`\nExample: `/opencode list files in current dir`", .{});
        defer ctx.allocator.free(error_msg);
    ...
    const result = opencode_module.sendOpenCodeMessage(ctx.allocator, opencode_msg) catch |err| {
        const error_msg = switch (err) {
            error.InvalidPrompt => "❌ Invalid prompt: Special characters are not allowed.\n\nAllowed: letters, numbers, spaces, and basic punctuation (.,:-_)",
            error.OpenCodeNotAvailable => "❌ OpenCode is not available. Please install it first.",
            else => try std.fmt.allocPrint(ctx.allocator, "❌ OpenCode error: {any}", .{err}),
        };
        defer if (err != error.InvalidPrompt and err != error.OpenCodeNotAvailable) ctx.allocator.free(error_msg);
    ...
    const tg_config = ctx.config.tools.telegram orelse return;
    sendMessage(ctx.allocator, ctx.client, tg_config.botToken, chat_id_str, result) catch |err| {
        std.debug.print("Failed to send response: {any}\n", .{err});
    };
}

/// Handle /opencodeconf command to show OpenCode configuration
fn handleOpencodeConfCommand(ctx: *TelegramContext, tg_data: TelegramTaskData) !void {
    const chat_id_str = try std.fmt.allocPrint(ctx.allocator, "{d}", .{tg_data.chat_id});
    defer ctx.allocator.free(chat_id_str);

    const tg_config = ctx.config.tools.telegram orelse return;

    const msg = "ℹ️ OpenCode configuration is handled by the OpenCode agent module.\nUse /opencode command to interact with OpenCode.";
    sendMessage(ctx.allocator, ctx.client, tg_config.botToken, chat_id_str, msg) catch |err| {
        std.debug.print("Failed to send response: {any}\n", .{err});
    };
}
...
pub fn handleTelegramTaskData(ctx: *TelegramContext, tg_data: TelegramTaskData) !void {
    ...
    // Check for /opencode command to send message to OpenCode
    if (std.mem.startsWith(u8, tg_data.text, "/opencode ")) {
        try handleOpencodeCommand(ctx, tg_data);
        return;
    }

    // Check for /opencodeconf command to show OpenCode configuration
    if (std.mem.eql(u8, tg_data.text, "/opencodeconf")) {
        try handleOpencodeConfCommand(ctx, tg_data);
        return;
    }
...
test "validateOpenCodePrompt: accepts valid prompts" {
    try opencode_module.validateOpenCodePrompt("hello world");
    try opencode_module.validateOpenCodePrompt("list files in current directory");
    try opencode_module.validateOpenCodePrompt("write a hello world program in python");
    try opencode_module.validateOpenCodePrompt("Hello, world! How are you?");
    try opencode_module.validateOpenCodePrompt("test-123_test");
    try opencode_module.validateOpenCodePrompt("simple command");
    try opencode_module.validateOpenCodePrompt("Code: foo-bar_baz");
    try opencode_module.validateOpenCodePrompt("");
}

test "validateOpenCodePrompt: rejects shell special characters" {
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("ls | grep foo"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("foo & bar"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("cmd; ls"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("echo $PATH"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("`ls`"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("test\\n"));
}

test "validateOpenCodePrompt: rejects quotes and brackets" {
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("echo \"hello\""));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("echo 'test'"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("test < file"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("test > output"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("foo(bar)"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("foo[0]"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("foo{bar}"));
}

test "validateOpenCodePrompt: rejects wildcards and operators" {
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("ls *"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("test~foo"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("# comment"));
}

test "validateOpenCodePrompt: rejects control characters" {
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("test\n"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("test\r"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("test\t"));
    try std.testing.expectError(error.InvalidPrompt, opencode_module.validateOpenCodePrompt("test\x00"));
}
